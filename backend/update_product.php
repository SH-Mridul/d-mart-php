...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['product_id'])) {
    ...
    $product_name = htmlspecialchars(trim($_POST['product_name']));
     ...
     if (empty($_POST['product_price'])) {
        $_SESSION['error'] = 'Product price is required.';
        header("Location: edit_product.php?id=$product_id");
        exit;
    }
    ...
    $update_product_query = "UPDATE products SET name = ?, price = ?, brand_id = ?, image_path = ? WHERE id = ?";
    $stmt = $conn->prepare($update_product_query);
    $stmt->bind_param("siisi", $product_name, $product_price, $brand_id, $image_path, $product_id);
